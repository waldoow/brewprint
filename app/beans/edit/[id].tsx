...
import { BeansService, type Bean, type BeanUpdate } from '@/lib/services/beans';
...
type FormData = z.infer<typeof beanUpdateSchema>;
...
export default function EditBeanScreen() {
  ...
  const {
    ...
    resolver: zodResolver(beanUpdateSchema),
    defaultValues: {
      name: '',
      roaster: '',
      origin: '',
  ...
  const loadBean = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        const beanData = result.data;
        setBean(beanData);
        
        // Reset form with bean data
        reset({
          name: beanData.name,
          ...
          roast_level: beanData.roast_level as any,
          weight_grams: beanData.weight_grams || 0,
          price: beanData.price || 0,
          altitude: beanData.altitude || 0,
          roast_date: beanData.roast_date?.split('T')[0] || '',
          purchase_date: beanData.purchase_date?.split('T')[0] || '',
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const result = await BeansService.updateBean(updateData);
      
      if (result.success) {
        toast.success('Bean updated successfully!');
        router.back();
      } else {
        toast.error(result.error || 'Failed to update bean');
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <ThemedView style={styles.container}>
        <Header title="Edit Bean" onBack={handleCancel} />
            ...
            <Controller
              ...
              render={({ field: { onChange, value } }) => (
                <ThemedInput
                  ...
                  value={value}
                  onChangeText={onChange}
                  placeholder="Floral, citrus, bright acidity..."
                  multiline
                  numberOfLines={4}
